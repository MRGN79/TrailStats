...
export function HrZones({ zones }: Props) {
  const { t } = useTranslation();

  if (!zones) return null;

  return (
    <section aria-label={t("stats.hrZones.title")}>
      <h2 className="section-title">{t("stats.hrZones.title")}</h2>
      <div className="pace-zones">
        {zones.zones.map((zd) => {
          const pct = Math.round(zd.share * 100);
          return (
            <div className="pace-zone" key={zd.zone}>
              <span className="pace-zone__label">{t(`stats.hrZones.${ZONE_KEYS[zd.zone - 1]}`)}</span>
      ...
      <p className="pace-zones__note">{t("stats.hrZones.note")}</p>
    </section>
  );
}
